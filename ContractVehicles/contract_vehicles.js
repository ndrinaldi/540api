...
function ValidatePOST(StrInput){
  ...
  var ValidKeys = ["name", "description", "id", "contract_path"];
...
function ValidatePATCH(StrInput){
  var ReturnVal = true;
  var input  = JSON.parse(StrInput);
  var ValidKeys = ["name", "description", "contract_path"];
  //Check for unregistered keys
  for(var key in input){ReturnVal = (ReturnVal && ValidKeys.includes(key))}
  return(ReturnVal);
}
...
module.exports.post = (event, context, callback) => {
  ...
  if(valid){
    ...
    const tempParams = {TableName: 'ContractVehicles' ,Key: {id: parseInt(JSON.parse(event.body).id)}}
    dynamoDb.get(tempParams, (error, result) => {
      if(result.Item == undefined){
        // Posting the item into DB
        const params = {TableName: 'ContractVehicles', Item: JSON.parse(event.body)}
        dynamoDb.put(params, (error, result) => {
          response = { // Success message sent
            statusCode: 201,
            body: JSON.stringify({"message":"Contract Vehicle was successfully created."})
          }
        ...
        response = { // Already posted message sent
          statusCode: 400,
          body: JSON.stringify({"message":"Contract Vehicle " + JSON.parse(event.body).id + " already exists."})
        }
        callback(null, response);
      }
    });
...
module.exports.list = (event, context, callback) => {
  ...
  const params = {TableName: 'ContractVehicles'}
...
module.exports.get = (event, context, callback) => {
  ...
  const params = {TableName: 'ContractVehicles', Key: {id: parseInt(event.pathParameters.id)}}
  dynamoDb.get(params, (error, result) => {
    if(result.Item == undefined) {
      response = { // Not ofund message sent
        statusCode: 404,
        body: JSON.stringify({"message":"Could not find Contract Vehicle " + event.pathParameters.id + "."})
      }
      callback(null, response);
    }else{
...
module.exports.put = (event, context, callback) => {
  ...
  if (valid) {
    ...
    const tempParams = {TableName: 'ContractVehicles', Key: {id: parseInt(event.pathParameters.id)}}
    dynamoDb.get(tempParams, (error, result) => {
      if(result.Item != undefined){
        ...
        response = { // Success message sent
          statusCode: 200,
          body: JSON.stringify({"message":"Contract Vehicle: " + event.pathParameters.id + " successfully replaced."})
        }
        callback(null, response)
      }else{
        response = { // Not found message sent
          statusCode: 404,
          body: JSON.stringify({"message": "Contract Vehicle " + event.pathParameters.id + " was not found."})
        }
        callback(null, response);
      }
    });
    ...
    dynamoDb.get(tempParams, (error, result) => {
      if(result.Item != undefined){
        ...
        response = { // Success return message sent
          statusCode: 200,
          body: JSON.stringify({"message":"Contract Vehicle: " + event.pathParameters.id + " successfully patched."})
        }
        callback(null, response);
      }else{
  ...
  dynamoDb.get(tempParams, (error, result) => {
    if(result.Item != undefined){
      ...
      dynamoDb.delete(params, (error, result) => {
        response = { // Success message sent
          statusCode: 200,
          body: JSON.stringify({"message": "Contract Vehicle: " + event.pathParameters.id + " successfully deleted."})
        }
        callback(null, response);
      });
    }else{
      response = { // Not found message sent
        statusCode: 404,
        body: JSON.stringify({"message": "Contract Vehicle " + event.pathParameters.id + " not found."})
      }
      callback(null, response);
    }
  });
