...
function ValidatePOST(StrInput){
  ...
  var ValidKeys = ["id", "apigeeRevokeUrl", "baseBaasUrl", "swaggerSpecUrl", "baseDamirProxyUrl", "phone", "email", "address", "githubURL", "twitterURL", "facebookURL", "linkedinURL"];
...
function ValidatePATCH(StrInput){
  var ReturnVal = true;
  var input  = JSON.parse(StrInput);
  var ValidKeys = ["apigeeRevokeUrl", "baseBaasUrl", "swaggerSpecUrl", "baseDamirProxyUrl", "phone", "email", "address", "githubURL", "twitterURL", "facebookURL", "linkedinURL"];
  //Check for unregistered keys
  for(var key in input){ReturnVal = (ReturnVal && ValidKeys.includes(key))}
  return(ReturnVal);
}
...
module.exports.post = (event, context, callback) => {
  ...
  if(valid){
    ...
    const tempParams = {TableName: 'Config' ,Key: {id: parseInt(JSON.parse(event.body).id)}}
    dynamoDb.get(tempParams, (error, result) => {
      if(result.Item == undefined){
        // Posting the item into DB
        const params = {TableName: 'Config', Item: JSON.parse(event.body)}
        dynamoDb.put(params, (error, result) => {
          console.log("ERROR: " + JSON.stringify(error));
          response = { // Success message sent
            statusCode: 201,
            headers: {"Access-Control-Allow-Origin": "*"},
            body: JSON.stringify({"message":"Config settings were successfully created."})
          }
        ...
        response = { // Already posted message sent
          statusCode: 400,
          headers: {"Access-Control-Allow-Origin": "*"},
          body: JSON.stringify({"message":"Config settings " + JSON.parse(event.body).id + " already exists."})
        }
        callback(null, response);
      }
    });
...
module.exports.get = (event, context, callback) => {
  ...
  const params = {TableName: 'Config', Key: {id: parseInt(event.pathParameters.id)}}
  dynamoDb.get(params, (error, result) => {
    if(result.Item == undefined) {
      response = { // Not ofund message sent
        statusCode: 404,
        headers: {"Access-Control-Allow-Origin": "*"},
        body: JSON.stringify({"message":"Could not find Config settings " + event.pathParameters.id + "."})
      }
      callback(null, response);
    }else{
...
module.exports.put = (event, context, callback) => {
  ...
  if (valid) {
    ...
    const tempParams = {TableName: 'Config', Key: {id: parseInt(event.pathParameters.id)}}
    dynamoDb.get(tempParams, (error, result) => {
      if(result.Item != undefined){
        ...
        response = { // Success message sent
          statusCode: 200,
          headers: {"Access-Control-Allow-Origin": "*"},
          body: JSON.stringify({"message":"Config settings " + event.pathParameters.id + " successfully replaced."})
        }
        callback(null, response)
      }else{
        response = { // Not found message sent
          statusCode: 404,
          headers: {"Access-Control-Allow-Origin": "*"},
          body: JSON.stringify({"message": "Config settings" + event.pathParameters.id + " was not found."})
        }
        callback(null, response);
      }
    });
    ...
    dynamoDb.get(tempParams, (error, result) => {
      if(result.Item != undefined){
        ...
        response = { // Success return message sent
          statusCode: 200,
          headers: {"Access-Control-Allow-Origin": "*"},
          body: JSON.stringify({"message":"Config settings " + event.pathParameters.id + " successfully patched."})
        }
        callback(null, response);
      }else{
    ...
    if(result.Item != undefined){
      ...
      dynamoDb.delete(params, (error, result) => {
        response = { // Success message sent
          statusCode: 200,
          headers: {"Access-Control-Allow-Origin": "*"},
          body: JSON.stringify({"message": "Partner  " + event.pathParameters.id + " successfully deleted."})
        }
        callback(null, response);
      });
    }else{
      response = { // Not found message sent
        statusCode: 404,
        headers: {"Access-Control-Allow-Origin": "*"},
        body: JSON.stringify({"message": "Config settings " + event.pathParameters.id + " not found."})
      }
      callback(null, response);
    }
  });
